def getAttributes(expression):
    ...
    expression = expression.split(
        "(")[1:]  # Remove the first '(' after the predicate
    expression = "(".join(expression)
    # Remove the last ')' after the last attribute
...
def unify(exp1, exp2):
    if exp1 == exp2:
        ...
        print(
            f"Predicates {getPredicate(exp1)} and {getPredicate(exp2)} do not match. Cannot be unified")
        return []
    elif len(getAttributes(exp1)) != len(getAttributes(exp2)):
        print(
            f"Length of attributes {len(getAttributes(exp1))} and {len(getAttributes(exp2))} do not match. Cannot be unified")
        return []
